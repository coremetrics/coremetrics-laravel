...
class EventBinder
{
    ...
    public function booting()
    {
       // dd((LARAVEL_START - $_SERVER['REQUEST_TIME_FLOAT']) * 1000);
        // -----> booting end
        $this->app['coremetrics.collector']->append(null, null, ['t' => TagCollection::REQUEST_STARTED], microtime(true));
    }
    ...
    public function bind()
    {
        ...
        {
            $this->app['coremetrics.collector']->append($event->connectionName, null, ['t' => TagCollection::TRANSACTION_BEGINNING]);
        });

        $this->app['events']->listen(TransactionCommitted::class, function(TransactionCommitted $event)
        {
            $this->app['coremetrics.collector']->append($event->connectionName, null, ['t' => TagCollection::TRANSACTION_COMMITED]);
        });

        $this->app['events']->listen(TransactionRolledBack::class, function(TransactionRolledBack $event)
        {
            $this->app['coremetrics.collector']->append($event->connectionName, null, ['t' => TagCollection::TRANSACTION_ROLLED_BACK]);
        });

        $this->app['events']->listen(QueryExecuted::class, function(QueryExecuted $event)
        {
            $this->app['coremetrics.collector']->append($event->sql, round($event->time, Collector::PRECISION), ['t' => TagCollection::QUERY]);
        });
        ...
        $this->app['events']->listen(RequestHandled::class, function(RequestHandled $event)
        {
            ...
            $this->app['coremetrics.collector']->append(null, $event->response->getStatusCode(), ['t' => TagCollection::REQUEST_HANDLED]);
        });
        ...
        $this->app->terminating(function()
        {
            ...
            $this->app['coremetrics.collector']->close();
        });
    }
}